...
from dataStore.SRC.butterworth import butterworth
import os
#df = pd.read_csv("DA500.csv")

def create_sr(df):
    df = butterworth(df)
    df2 = df[['velocity', 'time_of_day', 'distance','Vfilt2', 'split_d']].copy()
    x=df2.distance
    y=df2.Vfilt2
    v=df2.Vfilt2
    ...
    #move df to csv
    df4=df3[['velocity', 'distance', 'Vfilt2', 'SRMean3', 'split_d']]
